...
try:
    ...
    messagebox.showerror("Erro de Inicialização", 
                         f"Não foi possível carregar módulos de missão. Verifique os arquivos: {e}")
    sys.exit(1) 
...
class GameManager:
    ...
    def update_display(self):
        ...
        # --- FLUXO DAS MISSÕES EM SEQUÊNCIA ---
        # Missão 1: Agendamento de Intervalos Ponderados
            ...
            else: messagebox.showerror("Erro Crítico", "Classe Missao1 não foi carregada. Verifique imports.")
        ...
        # Missão 2: Maior Subsequência Crescente
            ...
            if 'Missao2' in globals():
                ...
                self.current_mission_obj.iniciar_missao_contexto()
            else: messagebox.showerror("Erro Crítico", "Classe Missao2 não foi carregada. Verifique imports.")
        elif self.game_state == "MISSION_2_SUCCESS":
            # Botão agora aponta para START_MISSION_3
            self._display_text_screen("Operação Concluída", ["Fulcrum: \"A evolução dos recrutas foi surpreendente. Sua lógica sequencial nos permitiu formar uma nova elite. Vamos ao próximo transporte crucial.\""], "Avançar para Missão 3", "START_MISSION_3", button_style="Accent.Dark.TButton", image_to_display=self.imagens.get("alianca_simbolo.png"))
        
        # Missão 3: Problema da Mochila (Knapsack)
        elif self.game_state == "START_MISSION_3":
            if 'Missao3' in globals():
                self._clear_content_frame()
                self.current_mission_obj = Missao3(self.root, self, self.content_frame) # Assegure-se que a classe se chama Missao3
                self.current_mission_obj.iniciar_missao_contexto()
            else: messagebox.showerror("Erro Crítico", "Classe Missao3 não foi carregada. Verifique imports.")
        ...
        # Missão 4: Edição de Distância (Levenshtein/Similaridade de Strings)
        elif self.game_state == "START_MISSION_4":
            if 'Missao4' in globals():
                self._clear_content_frame()
                self.current_mission_obj = Missao4(self.root, self, self.content_frame) # Assegure-se que a classe se chama Missao4
                self.current_mission_obj.iniciar_missao_contexto()
            else: messagebox.showerror("Erro Crítico", "Classe Missao4 não foi carregada. Verifique imports.")
        ...
        # Missão 5: Caminho Mais Curto em Grafos (Dijkstra/Bellman-Ford)
            ...
            if 'Missao5' in globals():
                ...
                messagebox.showerror("Erro Crítico", "Classe Missao5 não foi carregada. Verifique imports.")
        elif self.game_state == "MISSION_5_SUCCESS":
            # Leva para a tela final do jogo
        ...
        # Epílogo/Tela Final do Jogo
            ...
            self._display_text_screen(
                ...
                "Encerrar Jornada",
                self.root.quit, # Comando para fechar a aplicação
                button_style="Accent.Dark.TButton",
                image_to_display=None   # Remove a imagem de fundo para o epílogo
            )
            # Adiciona a imagem 'party.png' abaixo do botão, se existir
        ...
        # Estado de fallback para depuração
        else: 
            self._clear_content_frame()
    ...
    def set_game_state(self, new_state):
        ...
        self.root.after_idle(self.update_display) # Agenda a atualização da tela

    def mission_completed(self, mission_id):
        """Chamado pelas classes de missão quando uma missão é concluída com sucesso."""
        print(f"GameManager: Missão {mission_id} concluída.") 
        if mission_id == "Missao1": 
            self.set_game_state("MISSION_1_SUCCESS") 
        elif mission_id == "Missao2": 
            self.set_game_state("MISSION_2_SUCCESS")
        elif mission_id == "Missao3": 
            self.set_game_state("MISSION_3_SUCCESS")
        elif mission_id == "Missao4": 
            self.set_game_state("MISSION_4_SUCCESS")
        elif mission_id == "Missao5": 
            self.set_game_state("ALL_MISSIONS_COMPLETED_V3") # Missão 5 leva para o final do jogo
    
    def mission_failed_options(self, mission_obj, msg1, msg2):
        """Exibe opções ao jogador quando uma missão falha."""
        self._clear_content_frame()
        ...
        # Opção de tentar novamente, se a missão tiver um método retry_mission
...
if __name__ == "__main__":
    ...
    try:
        ...
        sys.exit(1) # Garante que o programa saia em caso de erro fatal0